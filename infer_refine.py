...
Supports 125M, 2B and 8B model variants for image editing with text instructions.
"""
import argparse
import logging
import os
...
def load_model(pretrain_root: str, model_path: str, model_size: str, image_size: int) -> Tuple[Any, ...]:
    ...
    if model_size not in MODEL_CONFIGS:
        raise ValueError(f"Unsupported model size: {model_size}. Choose '125M', '1B', '2B' or '8B'.")
    ...
    args_dict = {
        ...
        **config,
        ...
        'vae_path': f"{pretrain_root}/FoundationVision/Infinity/{config['vae_filename']}",
        'text_encoder_ckpt': f"{pretrain_root}/HiDream-ai/VAREdit/flan-t5-xl"
    }
    ...
    if image_size == 256:
        ...
        raise ValueError(f"Unsupported image size: {image_size}. Choose 256, 512 or 1024.")
...
def generate_image(
    ...
    assert image_size in [256, 512, 1024], f"Invalid image size: {image_size}, expected 256, 512 or 1024"
    if image_size == 256:
        pn = "0.06M"
    elif image_size == 512:
        pn = "0.25M"
    ...
    try:
        with Image.open(src_img_path) as src_img:
            ...
            src_img_tensor = transform(src_img, image_size, image_size)
    ...
    # Clear cache and collect garbage before generation
    torch.cuda.empty_cache()
    gc.collect()
...
def main():
    ...
    # pretrain_root = "weights"  # Pretrained model root directory
    # model_path = "/fs-ift/atlas/zouyuefeng/zls/code/VAR_IDP/weights/HiDream-ai/VAREdit/8B-512.pth"
    # model_size = "8B"
    # image_size = 512

    pretrain_root = "weights"  # Pretrained model root directory
    model_path = "/fs-ift/atlas/zouyuefeng/zls/code/VAR_IDP/local_output/run_97356972/0.25M-50K-ep19-iter1683.pth"
    model_size = "125M"
    image_size = 512
    ...
    input_folder = "data/assets/square_face"
    ...
    if model_size == "8B":
        ...
        COMMON_ARGS['bf16'] = 0
    output_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), "infer_output")
    ...
    if os.path.exists(prompt_json_path):
        ...
        logger.warning(f"Prompt file {prompt_json_path} not found. Using default prompt for all images.")
        prompts = {}
    
    default_prompt = "a person wearing a white hooded jacket with a black inner garment. The person has a neutral expression."
    ...
    for filename in os.listdir(input_folder):
        ...
        src_img_path = os.path.join(input_folder, filename)
        prompt = prompts.get(filename, default_prompt)
        
        logger.info(f"Processing {filename} with prompt: {prompt}")
