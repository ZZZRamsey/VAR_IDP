...
from transformers import AutoTokenizer, T5EncoderModel, T5TokenizerFast, AutoModel
...
def build_everything_from_args(args: arg_util.Args, saver):
    ...
    train_h_div_w_list = [1.0] # list(ld_train.dataset.h_div_w_template2generator.keys())
...
def build_model_optimizer(args, vae_ckpt):
    ...
    if args.rush_resume:
        if args.checkpoint_type == 'torch':
            ...
            load_sharded_checkpoint(gpt_wo_ddp, args.rush_resume, strict=False)
    ...
    # clip_model: AutoModel = AutoModel.from_pretrained(args.clip_path)
...
def main_train(args: arg_util.Args):
    ...
    for ep in range(start_ep, args.ep):
        ...
        #     ld_train.dataset.with_epoch(ep)
        # [train one epoch]
    ...
    with misc.Low_GPU_usage(files=[args.log_txt_path], sleep_secs=20, verbose=True) as telling_dont_kill:
        ...
        for it, data in me.log_every(start_it, iters_train, ld_or_itrt, args.log_freq, args.log_every_iter, header):
            ...
            with maybe_record_function('before_train'):
                ...
                # clip_features = clip_model.get_image_features(src_inp)
