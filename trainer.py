...
class InfinityTrainer(object):
    ...
    def train_step(
        ...
        T = 1 if target_inp_B3HW.dim() == 4 else target_inp_B3HW.shape[2]
        V = self.vae_local.vocab_size
        device = target_inp_B3HW.device

        h_div_w = target_inp_B3HW.shape[-2] / target_inp_B3HW.shape[-1]
        ...
        with self.gpt_opt.amp_ctx:
            ...
            # x_BLC_wo_prefix = []
            # start = 0
            # for _, h, w in scale_schedule[1:]:
            #     x_BLC_wo_prefix.append(source_x_BLC_wo_prefix[:, start:start+h*w])
            #     x_BLC_wo_prefix.append(target_x_BLC_wo_prefix[:, start:start+h*w])
            #     start += h * w
            # x_BLC_wo_prefix = torch.cat(x_BLC_wo_prefix, dim=1)
            ...
            gt_BL = torch.cat(target_gt_ms_idx_Bl, dim=1)[:,:training_seq_len].contiguous().type(torch.long) # [bs, 1*1+...+64*64, 16] or [bs, 1*1+...+64*64]
        ...
        if metric_lg.log_every_iter or it == 0 or it in metric_lg.log_iters:
            ...
            for scale_ind in range(min(training_scales, len(scale_schedule))):
                ...
                ptr = end
            grad_norm = grad_norm_t.item() if grad_norm_t is not None else 0
            metrics = torch.tensor(L_list + acc_bit_list + acc_token_list + [grad_norm, loss_token_mean, acc_bit_mean, acc_token_mean], device=loss.device)
