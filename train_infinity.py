...
def build_dataloaders(args):
    ...
    vbs = round(args.batch_size * 1.5)  # validation batch size, 验证时不需要反向传播，可以容纳更大的批次
    print(f"{args.batch_size=}, {vbs=}", flush=True)
    ld_val = math.ceil(50000 / vbs)
    ld_train = DataLoader(dataset=dataset_train, num_workers=args.workers, pin_memory=True, generator=args.get_different_generator_for_each_rank(), batch_size=None, prefetch_factor=args.prefetch_factor if args.workers > 0 else None)
