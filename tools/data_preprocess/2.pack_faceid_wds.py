...
def process_faceid_dataset(
    ...
    def process_one_file(json_file):
        ...
        try:
            ...
            # 验证图片是否可以正常打开
            # 注意：Image.open是lazy的，但在线程里最好确保它是完整的
            try:
                with Image.open(io.BytesIO(tgt_bytes)) as img:
                    img.convert('RGB')
                with Image.open(io.BytesIO(square_bytes)) as img:
                    img.convert('RGB')
            except Exception:
                return 'skipped', None
            ...
            res_data = {
                ...
                'height': height,
                'width': width,
                'modified_bboxes': meta_data.get("modified_bboxes", []),
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=32) as executor:
        ...
        results = executor.map(process_one_file, json_files)
        
        for status, data in tqdm(results, total=len(json_files), desc="处理样本"):
            # 如果达到指定的样本数量，停止处理
            ...
            if status == 'unqualified':
                ...
                try:
                    ...
                    meta = {
                        ...
                        "H": data['height'],
                        "W": data['width'],
                        "bbox_before_resize": data['modified_bboxes'],
                    }
